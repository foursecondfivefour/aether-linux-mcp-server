...
const RESOURCES: &[(&str, &str, &str)] = &[
    ...
    ("aether://system/memory", "memory_info", "Live memory metrics — total, used, available, buffer/cache, swap, hugepages"),
    ...
    ("aether://system/network-adapters", "network_adapters", "Network interfaces — IP addresses, MAC, speed, driver, statistics"),
    ...
    ("aether://system/security", "security_status", "Security subsystem status — SELinux/AppArmor, auditd, Secure Boot, kernel lockdown"),
    ...
    ("aether://system/installed-packages", "installed_packages", "List of installed packages via detected package manager"),
...
fn prompt_content(name: &str) -> String {
    match name {
        "system_health" => "Perform a comprehensive system health check on this Linux machine:\n\
            ...
            Summarize with a health score (1-10) and actionable recommendations.".to_string(),
        "security_posture" => "Audit the security posture of this Linux system:\n\
            ...
            Output a risk-scored report (Critical/High/Medium/Low) with remediation steps.".to_string(),
        "diagnostic_report" => "Generate a structured diagnostic report:\n\
            ...
            Format as structured report with severity and actionable remediation.".to_string(),
    ...
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            ...
            server_info: Implementation {
                name: "AETHER_02".to_string(),
                version: "1.0.0".to_string(),
            },
            instructions: Some(r#"AETHER_02 — Full-spectrum Linux management server.
...
"#.to_string()),
    ...
    async fn list_prompts(
        ...
        Ok(ListPromptsResult {
            prompts: PROMPT_INFO.iter().map(|(name, desc)| Prompt {
                name: name.to_string(),
    ...
    async fn list_resources(
        ...
        Ok(ListResourcesResult {
            resources: RESOURCES.iter().map(|(uri, name, desc)| Annotated {
                raw: RawResource {
                    uri: uri.to_string(),
                    name: name.to_string(),
                    description: Some(desc.to_string()),
    ...
    async fn read_resource(
        ...
        let key = request.uri.strip_prefix("aether://system/")
