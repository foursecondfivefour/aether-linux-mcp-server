...
impl AetherServer {
    ...
    async fn process_control(&self, Parameters(args): Parameters<ActionParams>) -> String {
        ...
        let raw = tools::process::handle(&self.gates, &action, args.params).await;
        tools::helpers::tool_response("process_control", &action, raw)
    }
    ...
    async fn file_system(&self, Parameters(args): Parameters<ActionParams>) -> String {
        let action = args.action;
        let raw = tools::filesystem::handle(&self.gates, &action, args.params);
        tools::helpers::tool_response("file_system", &action, raw)
    }
    ...
    async fn package_manager(&self, Parameters(args): Parameters<ActionParams>) -> String {
        let action = args.action;
        let raw = tools::packages::handle(&action, args.params);
        tools::helpers::tool_response("package_manager", &action, raw)
    }
    ...
    async fn system_config(&self, Parameters(args): Parameters<ActionParams>) -> String {
        let action = args.action;
        let raw = tools::sysconfig::handle(&self.gates, &action, args.params);
        tools::helpers::tool_response("system_config", &action, raw)
    }
    ...
    async fn service_manager(&self, Parameters(args): Parameters<ActionParams>) -> String {
        let action = args.action;
        let raw = tools::service::handle(&action, args.params);
        tools::helpers::tool_response("service_manager", &action, raw)
    }
    ...
    async fn gui_automation(&self, Parameters(args): Parameters<ActionParams>) -> String {
        let action = args.action;
        let raw = tools::gui::handle(&self.gates, &action, args.params);
        tools::helpers::tool_response("gui_automation", &action, raw)
    }
    ...
    async fn system_info(&self, Parameters(args): Parameters<ActionParams>) -> String {
        let action = args.action;
        let raw = tools::sysinfo::handle(&action, args.params);
        tools::helpers::tool_response("system_info", &action, raw)
    }
    ...
    async fn network_manager(&self, Parameters(args): Parameters<ActionParams>) -> String {
        let action = args.action;
        let raw = tools::network::handle(&self.gates, &action, args.params);
        tools::helpers::tool_response("network_manager", &action, raw)
    }
    ...
    async fn user_management(&self, Parameters(args): Parameters<ActionParams>) -> String {
        let action = args.action;
        let raw = tools::user::handle(&self.gates, &action, args.params);
        tools::helpers::tool_response("user_management", &action, raw)
    }
    ...
    async fn security_audit(&self, Parameters(args): Parameters<ActionParams>) -> String {
        let action = args.action;
        let raw = tools::security::handle(&self.gates, &action, args.params);
        tools::helpers::tool_response("security_audit", &action, raw)
    }
    ...
    async fn hardware_control(&self, Parameters(args): Parameters<ActionParams>) -> String {
        let action = args.action;
        let raw = tools::hardware::handle(&self.gates, &action, args.params);
        tools::helpers::tool_response("hardware_control", &action, raw)
    }
    ...
    async fn system_automation(&self, Parameters(args): Parameters<ActionParams>) -> String {
        let action = args.action;
        let raw = tools::automation::handle(&self.gates, &action, args.params);
        tools::helpers::tool_response("system_automation", &action, raw)
    }
}
