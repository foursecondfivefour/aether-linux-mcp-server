...
pub fn handle(gates: &FeatureGates, action: &str, params: serde_json::Value) -> String {
    ...
    let result = match action {
        ...
        "modprobe_load" => force_check(&params, &ctx).and_then(|_| gates.check(ctx.clone(), gates.module_load, "AETHER_MODULE_LOAD")).map(|_| run("modprobe", &[&ps(&params, "module", &ctx)])),
        "hostname_get" => Ok(fs::read_to_string("/proc/sys/kernel/hostname").unwrap_or_default()),
        "hostname_set" => force_check(&params, &ctx).map(|_| run("hostnamectl", &["set-hostname", &ps(&params, "hostname", &ctx)])),
        "timezone_list" => Ok(run("timedatectl", &["list-timezones"])),
        "timezone_get" => Ok(run("timedatectl", &[])),
        "timezone_set" => force_check(&params, &ctx).map(|_| run("timedatectl", &["set-timezone", &ps(&params, "timezone", &ctx)])),
